...
async function syncMinimis(jobName: string, runId: string, mode: 'initial' | 'daily' = 'initial') {
  ...
  while (!isLastPage) {
    ...
    try {
      ...
      totalErrors++;
      totalPagesFailed++;
      logger.error('Error de red al fetch minimis', { ...pageMeta, error: error.message });
      metrics.increment('etl.pages.failed');
      break;
    }
    ...
    for (const item of items) {
      ...
      try {
        ...
        totalErrors++;
        metrics.increment('etl.items.errors');
        logger.error('Error procesando minimis', { ...itemMeta, error: e.message });
      }
    }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    logger.error('Job Minimis Falló Catástroficamente', { jobName, runId, error: error.message, durationMs });
