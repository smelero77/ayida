  ...
  const renderMenuItem = (item: NavigationRootItem | NavigationNestedItem) => {
    ...
    if (item.items) {
          ...
          <Collapsible key={item.title} className="group/collapsible">
            <CollapsibleTrigger className="w-full flex justify-between items-center gap-2 px-2 py-1.5 [&[data-state=open]>svg]:rotate-180 relative cursor-default select-none rounded-sm text-sm outline-none data-[disabled=true]:pointer-events-none data-[selected='true']:bg-accent data-[selected=true]:text-accent-foreground data-[disabled=true]:opacity-50">
              <span className="flex items-center gap-2">
                {"iconName" in item && (
                  <DynamicIcon name={item.iconName as any} className="h-4 w-4" />
                )}
                <span>{title}</span>
    ...
    if ("href" in item) {
      ...
      return (
        ...
        >
          {"iconName" in item ? (
            <DynamicIcon name={item.iconName as any} />
          ) : (
